...
function useResizeObserverStore(
	...
	const onSubscribe = React.useCallback((callback: () => void) => resizeObserverStore.subscribe(callback), [])
...
function Marquee(props: MarqueeProps) {
	...
	const contextValue = React.useMemo<MarqueeContextValue>(
		...
		[side, orientation, dir, speed, loopCount, autoFill, paused, pauseOnHover, pauseOnKeyboard, reverse]
	)
	...
	return (
		<>
			<ContentPrimitive
				...
				<div ref={composedRef} className={cn('flex shrink-0 gap-(--marquee-gap)', isVertical && 'flex-col')}>
					{children}
				</div>
...
function MarqueeItem(props: DivProps) {
	...
	return <ItemPrimitive data-slot='marquee-item' {...itemProps} className={cn('shrink-0', className)} />
}
